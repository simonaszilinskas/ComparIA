...
The whole app runs on Railway as five services in the `comparia-sante` project
(workspace: The Safe AI), mirroring the topology of
...
- **Managed Postgres.** Full connection URL works directly; the app rewrites
...
Set on the Railway dashboard or CLI (secrets come from the local `.env`):

- **backend**: `COMPARIA_DB_URI=${{Postgres.DATABASE_URL}}`,
  `COMPARIA_REDIS_HOST=redis.railway.internal`, `OPENROUTER_API_KEY`,
  `SCALEWAY_API_KEY`, `ALTCHA_HMAC_KEY`, `LOG_FORMAT=JSON`, `LOGDIR=/data`,
  `CACHE_ENABLED=true`, `CACHE_PROBABILITY=0.75`, `DEFAULT_COUNTRY_PORTAL=fr`.
